...
function SearchResult({ item, onOpenSubmission }) {
  return <article className="search-result-card">
    ...
    <footer>
      <span>{item.document_title || '未命名面经'} · {item.round_label || `第 ${item.round_ordinal || '—'} 轮`} · 原文块 {item.chunk_ordinal}</span>
      <span>{item.host || '手工录入正文'} · {shortDate(item.collected_at)}</span>
      ...
      {item.submission_id && <button type="button" onClick={() => onOpenSubmission(item.submission_id)}>查看情报详情</button>}
    </footer>
  </article>
}

export function IntelligenceSearchPanel({ navigate }) {
  ...
  async function submit(event) {
    ...
    if (!q.trim()) return
    await search({ q, roundOrdinal, fieldKind, sourceHost }).catch(() => null)
  }

  return <section className="intelligence-search-panel">
    <header className="search-panel-heading">
      <div><span>检索工作区</span><h2>设置查询范围</h2><p>精确术语优先，FTS 与 trigram 只提供可解释候选；每条结果都能回到来源与原文位置。</p></div>
      <div className={`quality-stamp ${quality?.status === 'insufficient_data' ? 'limited' : ''}`}>
        <ShieldCheck size={16} /><span>{qualityLoading ? '正在读取质量状态' : quality?.status === 'insufficient_data' ? '当前数据量有限' : '质量状态已读取'}</span>
        <small>{quality?.conclusion || '尚未读取质量状态'}</small>
      </div>
    </header>
    <form className="search-controls" onSubmit={submit}>
      <label className="search-query"><span>查询内容</span><div><Search size={17} /><input value={q} onChange={(event) => setQ(event.target.value)} placeholder="例如：如何排查线上慢查询？" /></div></label>
      <label><span>面试轮次</span><input inputMode="numeric" value={roundOrdinal} onChange={(event) => setRoundOrdinal(event.target.value)} placeholder="全部" /></label>
      <label><span>问题类型</span><select value={fieldKind} onChange={(event) => setFieldKind(event.target.value)}><option value="">全部问题</option><option value="question">主问题</option><option value="follow_up">追问</option></select></label>
      <label><span>来源域名</span><input value={sourceHost} onChange={(event) => setSourceHost(event.target.value)} placeholder="cnblogs.com" /></label>
    ...
    <div className="search-result-stack">
      ...
      {result.results.map((item) => <SearchResult item={item} onOpenSubmission={(id) => navigate(`/intelligence/${id}`)} key={item.occurrence_id} />)}
    </div>
  </section>
}
