...
export function CanonicalQuestionPanel({ selectedId, navigate }) {
  const { questions, selected, loading, error, refresh, select, clearSelected, merge, split, remap } = useCanonicalQuestions()
  const [mergeTarget, setMergeTarget] = useState('')
  const [busy, setBusy] = useState(false)
  const occurrenceTotal = questions.reduce((sum, item) => sum + item.occurrence_count, 0)

  useEffect(() => {
    if (!selectedId) { clearSelected(); return }
    if (selectedId && selected?.id !== selectedId) select(selectedId).catch(() => null)
  }, [selectedId, selected?.id, select, clearSelected])
  ...
  async function mergeQuestion() {
    ...
    try {
      ...
      await merge(selected.id, target)
      setMergeTarget('')
      navigate(`/intelligence/questions/${target}`, { replace: true })
    } finally { setBusy(false) }
  }

  return <section className="canonical-panel">
    <header>
      <div><span>规范化工作区</span><h2>核对题目与证据</h2><p>选择一条规范题，查看它的原始出现位置与人工映射历史。</p></div>
    ...
    <div className="canonical-body">
      <aside className="canonical-list">
        ...
        {questions.map((item, index) => <button key={item.id} className={selected?.id === item.id ? 'active' : ''} onClick={() => navigate(`/intelligence/questions/${item.id}`)}>
      ...
      <article className="canonical-detail">
        {!selected ? <div className="canonical-empty"><Sigma size={28} /><h3>选择规范题核对证据</h3><p>每个计数都可下钻至文档、轮次、内容块与 evidence span。</p></div> : <>
          <header><span>{selected.created_by === 'manual' ? '人工建立的规范题' : '确定性规则生成的规范题'}</span><h3>{selected.canonical_text}</h3></header>
          ...
          <div className="occurrence-stack">{selected.occurrences.map((item) => <section className="occurrence-card" key={item.id}>
            <div><span>{item.round_label || '轮次待确认'} · 原文块 {item.chunk_ordinal}</span><strong>{item.raw_text}</strong><small>{item.document_title || '未命名面经'} · {shortDate(item.collected_at)}</small></div>
