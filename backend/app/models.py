...
class KnowledgeCard(Base):
    ...
    notes: Mapped[str | None] = mapped_column(Text)
    mastery_status: Mapped[str] = mapped_column(String(24), nullable=False, server_default=text("'not_started'"))
...
class KnowledgeCardEvidence(Base):
    ...
    card_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("knowledge_cards.id", ondelete="CASCADE"), primary_key=True)
