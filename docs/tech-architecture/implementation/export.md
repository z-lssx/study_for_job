...
状态：T016 已实现同步只读导出链路（2026-08-06）。
...
T014 assessment 是规则投影，不是导出事实源，因此未作为主体导出，也不会替代上述底层业务事实。
...
按用户最高优先级规则，T016 没有编写或执行测试、测试脚本、静态语法/类型/lint 检查、迁移验证、API/数据库运行态验证、页面行为/浏览器/截图验收或构建。只完成集合/关系覆盖、两格式同源、排序/指纹、分类边界、证据正文上限、主动触发、空错态和非范围的逻辑检查。
