...
- `internship_intelligence_links` 显式关联规范题，可选绑定同一实习事实，并返回 occurrence 次数与最多三条原始面经证据回链。
...
桌面“实习资产”入口提供经历选择、事实修订、STAR 版本历史、材料状态和规范题/原文来源回链。列表、详情和长编辑使用当前工作台统一视觉层级，但不得改变本轨道数据流。
