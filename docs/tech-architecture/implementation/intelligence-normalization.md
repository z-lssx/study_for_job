...
T006 产生的 `question_candidates` 是原文抽取候选，不直接承担规范题身份或频率事实。T007 增加一个保守的规范题层：确定性归一化负责可解释的初始聚合，原始出现记录负责计数，人工映射负责合并、拆分和等价修正。三者分层后，用户修订不会改写候选文本、字符区间或 evidence span。
...
已验证：迁移 006 在 development/usage 应用；刷新首次创建 2 条 occurrence、2 个规范题和 2 个映射，重复刷新创建数为 0；频率列表和详情 API 可用；回滚事务内人工映射、合并和拆分不改变 occurrence 原始字段；usage 的三张 T007 业务表计数为 0/0/0；Vite 开发服务器可解析新增模块。

当前限制：尚未实现岗位/公司维度过滤、语义相似度、embedding、频率快照和批量治理；浏览器 CLI 未安装，未完成真实点击式页面验收。T005 已登记的 robots/HTTPS/代理/DNS/隐藏文本/hash 风险仍是延期风险，T007 未修改也未解决。
