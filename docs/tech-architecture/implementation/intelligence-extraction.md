...
抽取输入只读取成功文档的清洗纯文本、清洗版本和内容 hash，不复制 raw/cleaned 正文。轮次和块顺序由确定性行规则生成；轮次不明确时块仍保留且 `round_id` 为空，无法分类的块为 `unknown`/`needs_review`。
...
历史实现验证覆盖确定性抽取和 handler 契约。真实远程 DeepSeek、PostgreSQL/Worker 端到端和页面行为不属于当前已确认运行事实。此链路不提供规范题归一化、频率统计或语义检索。
