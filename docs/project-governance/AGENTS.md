...
阶段完成后，阶段总管应停止扩展当前计划，先按 `DOCUMENT_LIFECYCLE.md` 完成阶段文档收口，再输出交接文档并主动发送给来源项目大总管。交接至少包括当前阶段、已完成事项、进行中任务及 ID、未决事项、风险、下一步和必读文档；不得自行创建下一阶段总管。若当前项目尚未启用大总管或来源大总管已不可用，再由用户按交接文档手动接力。
